    ...
    public void run() {
        ...
        for (int j = 0; j < dataList.size(); j++) {
            try {
                ...
                for (int i = 0; i < dataList.get(j).getLike().length; i++) {
                    loadDataLike(dataList.get(j).getBv(),dataList.get(j).getLike()[i]);
                }
                for (int i = 0; i < dataList.get(j).getCoin().length; i++) {
                    loadDataCoin(dataList.get(j).getBv(),dataList.get(j).getCoin()[i]);
                }
                for (int i = 0; i < dataList.get(j).getFavorite().length; i++) {
                    loadDataFavorite(dataList.get(j).getBv(),dataList.get(j).getFavorite()[i]);
                }
                for (int i = 0; i < dataList.get(j).getViewerMids().length; i++) {
                    loadDataView(dataList.get(j).getBv(),dataList.get(j).getViewerMids()[i],dataList.get(j).getViewTime()[i]);
                }
                num++;
                if (num %30 == 0) {
                    try {
